...
public class StreamDetailTest {
    ...
    public static void setRandom(Random random) {
        StreamDetailTest.random = random;
    }
    ...
    public static void main(String[] args) {
        StreamDetailTest streamDutailTest = new StreamDetailTest();
        streamDutailTest.data();
        List<Order> orders = streamDutailTest.getOrders();
        ...
        System.out.println(orders.stream().collect
                (groupingBy(order -> order.getPlacedAt().format(DateTimeFormatter.ofPattern("yyyyMM")),
                        groupingBy(order -> order.getCustomerName(),
                                mapping(order -> order.getId(), toList())))));
//        Map<String, Map<String, List<Long>>> yyyyMM =
//        Optional<Entry<String, List<Long>>> yyyyMM =
//        List<Entry<String, List<Long>>> yyyyMM =
        List<Long> yyyyMM = orders.stream().collect
            (groupingBy(order -> order.getPlacedAt().format(DateTimeFormatter.ofPattern("yyyyMM")),
                groupingBy(order -> order.getCustomerName(),
                    mapping(order -> order.getId(), toList())))).entrySet().stream()
            .flatMap((entry) ->
                entry.getValue().entrySet().stream()
            ).flatMap(ttent -> ttent.getValue().stream()).collect(toList());
